...
const NewTeacher = (props) => {
    ...
    const submitHandler = (event) => {
        ...
        if (
            !fullnameRef.current || fullnameRef.current.value.trim().length === 0 ||
            !birthdayRef.current || birthdayRef.current.value.trim().length === 0 ||
            !higherEducationRef.current || higherEducationRef.current.value.trim().length === 0 ||
            !specialityRef.current || specialityRef.current.value.trim().length === 0 ||
            !qualificationRef.current || qualificationRef.current.value.trim().length === 0 ||
            !degreeRef.current || degreeRef.current.value.trim().length === 0 ||
            !academicStatusRef.current || academicStatusRef.current.value.trim().length === 0 ||
            !jobRef.current || jobRef.current.value.trim().length === 0 ||
            !positionRef.current || positionRef.current.value.trim().length === 0 ||
            !professionalInterestsRef.current || professionalInterestsRef.current.value.trim().length === 0 ||
            !topicsRef.current || topicsRef.current.value.trim().length === 0 ||
            !phoneNumberRef.current || phoneNumberRef.current.value.trim().length === 0 ||
            !emailRef.current || emailRef.current.value.trim().length === 0 ||
            !facebookRef.current || facebookRef.current.value.trim().length === 0
        ) {
    ...
    return <form className={styles.form}>
       ...
       <div className={styles.control}>
            <label htmlFor="birthday">Число, місяць і рік народження (в форматі дд.мм.рр)</label>
            <input type="text" id="birthday" ref={birthdayRef}/>
       </div>
       <div className={styles.control}>
            <label htmlFor="education">Який вищий навчальний заклад або заклади закінчив/ла?</label>
            <input type="text" id="education" ref={higherEducationRef}/>
        </div>
        <div className={styles.control}>
            <label htmlFor="speciality">Спеціальність за дипломом (дипломами)</label>
        ...
        <div className={styles.control}>
            <label htmlFor="topics">Які теми читаєте (з анотаціями в 3 – 5 речень)</label>
        ...
        <button className={styles.button} onClick={submitHandler}>Додати викладача</button>
