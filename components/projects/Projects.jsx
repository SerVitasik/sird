...
const Projects = ({ projects }) => {
  ...
  const [filteredByContentProjects, setFilteredProjects] = useState(projectsWithCorrectDate);
  const [selectedDate, setSelectedDate] = useState("");

  const filteredDateProjects =
    selectedDate === ""
      ? projectsWithCorrectDate
      : projectsWithCorrectDate.filter(
          (item) => {
  ...
  const filtered = filteredByContentProjects.filter((projectsObject) => {
    return filteredDateProjects.some(element => JSON.stringify(element) === JSON.stringify(projectsObject));
  });
  const handleSearchChange = (e) => {
    setSearchTerm(e.target.value);
    setTimeout(() => {
      const filteredByContent = projectsWithCorrectDate.filter(
        (item) => {
